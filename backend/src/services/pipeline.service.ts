...
export class PipelineService {
  async buildFromGitUrl(gitUrl: string, deploymentId: string): Promise<{ imageTag: string }> {
  ...
  async buildFromDirectory(sourceDir: string, deploymentId: string): Promise<{ imageTag: string }> {
    const buildDir = path.join(BUILD_DIR, deploymentId);
  ...
  async runContainer(imageTag: string, deploymentId: string): Promise<{ port: number; liveUrl: string }> {
    ...
    try {
      ...
      logEmitterService.emitLog(deploymentId, `[DEPLOY] Warning: Caddy route setup failed: ${(error as Error).message}`);
    }
  ...
  async stopContainer(deploymentId: string): Promise<void> {
    ...
    try {
      ...
      logEmitterService.emitLog(deploymentId, `[DEPLOY] Warning: Failed to stop container: ${(error as Error).message}`);
    }
  ...
  async getContainerLogs(deploymentId: string, lines = 100): Promise<string> {
    const deployment = deploymentRepository.findById(deploymentId);
    if (!deployment?.containerId) return 'No container running for this deployment.';
  ...
  private async cloneRepo(gitUrl: string, targetDir: string, deploymentId: string): Promise<void> {
    ...
    if (stdout) logEmitterService.emitLog(deploymentId, `[BUILD] ${stdout}`);
    if (stderr) logEmitterService.emitLog(deploymentId, `[BUILD] ${stderr}`);
  }

  private async runRailpack(buildDir: string, deploymentId: string): Promise<string> {
    ...
    const { stdout, stderr } = await execa(
      railpackBinary,
      ['build', buildDir, '--name', imageTag, '--progress', 'plain'],
      {
        cwd: buildDir,
        env: {
